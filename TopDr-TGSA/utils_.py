...
def multiomics_data():
    RNAseq = np.genfromtxt("{}{}.csv".format(path, "cn"), delimiter=',', skip_header=1, dtype=np.dtype(str))
    ...
    drugtemp = []
    lablefile = csv.reader(open("{}{}.csv".format(path, "PANCANCER_IC_82833_580_170"), 'r'))
    lablefile = list(lablefile)
    drug_cell_lable = []
    for i in range(1, len(lablefile)):
        drug_cell_lable.append(lablefile[i][0:1]+lablefile[i][2:3]+lablefile[i][9:10])
...
def sim_graph(omics_data, cell_number):
    ...
    for i in range(cell_number):
        ...
        for k in top_indices_2:
            rows_10.append(i)
            cols_10.append(k)
            values_10.append(sim_matrix[i,k])  # 边的权重
...
def data_index(cell, drug, lable):
    Reglable = np.array(lable[:,2], dtype = float)
    sample_set = []
