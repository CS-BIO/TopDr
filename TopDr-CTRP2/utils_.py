...
def multiomics_data():
    RNAseq = np.genfromtxt("{}{}.csv".format(path, "CTRP2_RNAseq"), delimiter=',', skip_header=1, dtype=np.dtype(str))
    ...
    fingerprint = pd.read_csv("{}{}.csv".format(path, "CTRP2_fingerprint_881dim"),  skiprows=0)
    ...
    lablefile = csv.reader(open("{}{}.csv".format(path, "CTRP2_response"), 'r'))
    ...
    for src, dst_list in dict_adj.items():
        for dst in dst_list:
            if dst > src:
                rows.append(src)
                cols.append(dst)
                edge_weights.append(1/sim_mat[src, dst])  # 使用相似度倒数作为边权重
