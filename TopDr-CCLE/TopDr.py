...
# cell_adj_matrix, cell_feat_matrix = cell_adj_matrix.to(device), cell_feat_matrix.to(device)
# drug_adj_matrix, drug_feat_matrix = drug_adj_matrix.to(device), drug_feat_matrix.to(device)
...
# Model and optimizer 
model = TopDr(ncell = ncell, ndrug = ndrug,  ncellfeat = ncellfeat,  ndrugfeat = ndrugfeat,  nhid=args.hidden, nheads=args.nb_heads, alpha=args.alpha)
